...
## حل مشكلة ظهور علامة التارقت بدون استجابة
إذا ظهر لك الـ Target لكن ما يفتح الخيارات أو ما يشتغل الضغط:
1. تأكد أن شخصية اللاعب وظيفتها `gascompany`.
2. تأكد أن `Config.Target` مطابق للنظام المركب فعلاً (`qb-target` أو `ox_target`).
3. تأكد من ترتيب التشغيل في `server.cfg`: شغّل target + `ox_lib` قبل `qb-gascompany`.
4. نفّذ `ensure qb-gascompany` بعد أي تعديل.
5. راقب F8/console لأي أخطاء exports خاصة بـ target.
